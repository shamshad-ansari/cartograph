...
// The self-contained result of parsing one file (Phase A): the local facts a
// worker extracts with no access to the shared graph (ADR-0006). Facts carry
// names and source locations only — never NodeIDs — so nothing is stitched into
// the graph until the sequential merge (Phase B).
struct ParsedFile {
...
// Phase A — parse. Read, parse, and extract every file's local facts in
// parallel across `workers`. The crawl order in `paths` fixes each file's slot
// in the result, so the merge that follows is deterministic regardless of which
// worker finished first. Each worker owns a thread-local Parser (a tree-sitter
// parser is single-threaded) and writes only its own `parsed[i]` — share-nothing,
// so no lock is needed.
std::vector<ParsedFile> parse_files(const std::vector<std::filesystem::path>& paths,
                                    unsigned workers) {
  ...
  parallel_for(paths.size(), workers, [&](std::size_t i) {
    ...
    // parser; rather than index half-parsed garbage we flag it and let the merge
    // record the skip, so a single bad file doesn't poison the whole index.
    if (tree.has_error()) {
...
Graph index_directory(const std::filesystem::path& dir, const IndexOptions& opts) {
  ...
  // ── Crawl ──────────────────────────────────────────────────────────────
  // Collect the C sources under `dir` in a single sequential walk. The error-code
  // overloads plus skip_permission_denied keep the crawl from throwing on an
  // unreadable subdirectory, so one bad entry never aborts indexing. This order
  // is the graph's canonical order — Phase B assigns NodeIDs by walking it — so
  // it is fixed here, once, before any parallelism.
  ...
  // ── Phase A — parse (parallel) ─────────────────────────────────────────
  const std::vector<ParsedFile> parsed = parse_files(paths, workers);
